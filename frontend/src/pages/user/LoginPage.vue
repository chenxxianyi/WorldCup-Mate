  ...
  <div class="page-view login-wrap">
    <section class="login-panel">
      <div class="login-visual">
        <span class="hero-mark">{{ theme.current.mark }}</span><div><h1>你的比赛日，应该更简单。</h1><p>登录后同步关注球队、收藏比赛与开球提醒，在六个赛事世界之间无缝切换。</p></div><small>{{ theme.current.name }} · {{ theme.current.season }}</small>
      </div>
      <form
        class="login-form"
      ...
      >
        <p class="eyebrow">
          {{ isRegister ? 'JOIN MATCHDAY' : 'WELCOME BACK' }}
        </p><h2>{{ isRegister ? '注册' : '登录' }} WorldCup Mate</h2><p>登录后同步你的关注、收藏和提醒设置。</p>
        <div class="form-stack">
          <label
            v-if="isRegister"
            class="field-label"
          >用户名<div class="field-control"><ThemeIcon name="user" /><input
            v-model="username"
            type="text"
            ...
            required
          ></div></label>
          <label class="field-label">邮箱<div class="field-control"><ThemeIcon name="mail" /><input
            v-model="email"
            type="email"
            autocomplete="email"
            required
          ></div></label>
          <label class="field-label">密码<div class="field-control"><ThemeIcon name="lock" /><input
            v-model="password"
            type="password"
          ...
          <div class="form-row">
            ...
            > 记住我</label><button
              class="text-button"
              type="button"
              @click="theme.showToast('重置密码邮件功能待接入')"
            >
              忘记密码？
            </button>
          </div>
          ...
          <button
            ...
            :disabled="loading"
          >
            {{ loading ? '处理中…' : isRegister ? '注册并继续' : '登录并继续' }} <ThemeIcon name="arrow" />
          </button>
          <button
