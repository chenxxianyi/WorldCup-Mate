...
<template>
  <div class="page-view">
    <header class="page-heading">
      <div>
        <p class="eyebrow">
          {{ theme.current.en }} · {{ theme.competition.current?.season || theme.current.season }}
        </p><h1>积分榜</h1>
      </div><div class="standings-tabs">
        <button
          v-for="item in modes"
          :key="item"
          class="chip"
    ...
    <div
      ...
      class="page-state"
    >
      <span class="state-spinner" />正在加载积分榜
    </div>
    <section
    ...
    >
      <table class="standings-table">
        <thead><tr><th>排名</th><th>球队</th><th>赛</th><th>胜</th><th>平</th><th>负</th><th>净胜</th><th>积分</th></tr></thead><tbody>
          <tr
            v-for="row in rows"
            :key="row.teamId"
          >
            <td>
              ...
              <span class="table-team"><TeamBadge
                :team="badge(row)"
                size="small"
              /><span>{{ row.name }}</span></span>
            </td><td>{{ row.played }}</td><td>{{ row.won }}</td><td>{{ row.drawn }}</td><td>{{ row.lost }}</td><td>{{ row.goalDifference > 0 ? '+' : '' }}{{ row.goalDifference }}</td><td><strong>{{ row.points }}</strong></td>
          </tr>
        </tbody>
      </table>
      <div class="zone-legend">
        <span class="zone-item"><i class="zone-swatch" />晋级 / 欧冠区</span><span class="zone-item"><i class="zone-swatch europa" />次级欧战区</span><span class="zone-item"><i class="zone-swatch danger" />淘汰 / 降级区</span>
      </div>
    </section>
    <article
      v-else
    ...
    >
      <span class="empty-art"><ThemeIcon name="trophy" /></span><span class="empty-copy"><h3>暂无积分数据</h3><p>{{ error || '完成比赛同步或积分重算后将在这里展示。' }}</p></span>
    </article>
  </div>
</template>
