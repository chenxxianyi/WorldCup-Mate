...
function Run-Step([string]$Name, [scriptblock]$Command) {
    ...
    try {
        $output = & $Command 2>&1
        # $LASTEXITCODE is null when the child never set one: treat as success.
        $code = if ($LASTEXITCODE -ne $null) { $LASTEXITCODE } else { 0 }
    } catch {
        # A throwing channel is a failed channel: record it and let the
        # caller fall through to the next one (never abort the whole route).
        $output = "channel failed: $($_.Exception.Message)"
        $code = 1
    }
...
if ($Intent -eq 'ocr') {
    ...
    if ($ext -in $imageExts) {
        # Windows OCR may be unavailable (missing OCR language pack, older
        # Windows): Run-Step records the throw and we continue the chain.
...
if ($Intent -eq 'reason') {
    ...
    # Hashtable splatting: PS 5.1 array splatting mis-binds arguments when
    # the callee has switch params with defaults (ImagePath received the
    # literal '-ImagePath').
    $baseArgs = @{
        ImagePath = $Path
        Prompt    = $Prompt
